...
import { getProfile, loadLocal, saveLocal, clearLocal, updateStats, BET_AMOUNTS, saveGameHistory, deductTokens, updateBalance, getPendingRequests, getGameInvites, subscribeToInvites, joinRoom } from './services/gameService'
import type { User, GameConfig, GameInvite, Question } from './types'
...
export default function App() {
  ...
  const [gamePlayers, setGamePlayers] = useState<User[]>([])
      ...
      try {
        ...
        if (session?.user) {
          ...
          if (profile) {
            ...
            loadGameInvites(session.user.id)
          }
        }
      } catch (e) {
  ...
  const handlePlay = async (cfg: GameConfig) => {
    ...
    // Списываем токены
    if (isOnline && cfg.betAmount > 0) {
      const success = await deductTokens(user.id, cfg.betAmount)
      if (!success) {
        alert('Недостаточно токенов!')
        return
      }
      setUser({ ...user, balance: user.balance - cfg.betAmount })
    }
    
    // Для онлайн режимов - комната ожидания
    if (cfg.roomId) {
      setWaitingRoomId(cfg.roomId)
      setIsHost(cfg.isHost || false)
      setConfig(cfg)
      setView('waiting')
      return
    }
  ...
  const handleGameStart = (questions: Question[], players: User[]) => {
  ...
  const handleGameEnd = async (won: boolean, correct: number, wrong: number, totalPlayers: number, surrendered: boolean) => {
    ...
    if (isOnline) {
      ...
      await saveGameHistory({
        user_id: user.id, 
        room_id: config.roomId || 'local-' + Date.now(), 
        mode: config.mode,
      ...
      setUser({
        ...
        level: Math.floor((user.total_games + 1) / 5) + 1
      })
    }
  ...
  const handleInviteAccept = async (roomId: string) => {
    if (!user) return
  ...
  const handleInviteDecline = () => {
    ...
    if (remaining.length > 0) setCurrentInvite(remaining[0])
  }
      ...
      <div className="relative z-10 min-h-screen">
        ...
        {view === 'game' && user && config && (
          <Game 
            ...
            onlinePlayers={gamePlayers}
          />
        )}
        
        {view === 'profile' && user && (
          <Profile user={user} isOnline={isOnline} onBack={() => setView('dashboard')} onUserUpdate={setUser} />
        )}
        
        {view === 'friends' && user && (
          <Friends user={user} isOnline={isOnline} onBack={() => { loadPendingRequests(user.id); setView('dashboard') }} />
        )}
        
        {view === 'admin' && user?.is_admin && (
          <AdminPanel user={user} onBack={() => setView('dashboard')} />
        )}
      </div>
    </div>
  )
