...
// Получить открытые комнаты (Арена Битв)
export const getOpenRooms = async (): Promise<GameRoom[]> => {
  const { data } = await supabase
    .from('game_rooms')
    .select('*, profiles!game_rooms_host_id_fkey(username, avatar_url)')
    .eq('status', 'waiting')
  ...
  return data.map(room => ({
    ...room,
    host_username: room.profiles?.username,
    host_avatar: room.profiles?.avatar_url
  }))
}

// Подписка на открытые комнаты
export const subscribeToOpenRooms = (callback: (rooms: GameRoom[]) => void) => {
  return supabase
    .channel('open-rooms')
    .on('postgres_changes', {
      event: '*',
...
export const createGameRoom = async (
  ...
  if (invitedFriends && invitedFriends.length > 0) {
    for (const friendId of invitedFriends) {
      await sendGameInvite(roomId, hostId, friendId)
    }
  }
...
export const joinRoom = async (roomId: string, userId: string): Promise<boolean> => {
  // Проверяем что комната ещё открыта
  ...
  await supabase.from('game_rooms').update({ 
    player_count: (room.player_count || 1) + 1 
  }).eq('id', roomId)
